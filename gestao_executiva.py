...
class GestaoExecutiva:
    ...
    def criar_dashboard_executivo(self, mes_selecionado=None):
        ...
        with col4:
            status_icon = "✅" if resultado['status_quitacao'] else "⏳"
            st.metric(
                label=f"{status_icon} Status do Mês",
                value="Quitado" if resultado['status_quitacao'] else "Pendente"
            )
        ...
        # Adicionar barras
        colors = {'🟢 Receitas': '#00CC88', '🔴 Despesas': '#FF4444', '🔵 Saldo': '#3366CC'}
        ...
        for categoria in df_grafico['Categoria'].unique():
            ...
            fig.add_trace(go.Bar(
                ...
                x=df_cat['Categoria'],
                y=df_cat['Valor'],
                marker_color=colors.get(categoria, '#3366CC'),
